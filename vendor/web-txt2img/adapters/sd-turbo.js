...
export class SDTurboAdapter {
    ...
    async load(options) {
        ...
        try {
            ...
            if (!ort) {
                let ortMod = null;
                // LLM: Same as Janus — bare onnxruntime-web/* fails in module workers when import maps do not apply; use absolute +esm/bundle URLs (pin with diffusers-webgpu-compare-test.html import map).
    ...
    async generate(params) {
        ...
        try {
            ...
            // LLM: SD UNet expects CLIP seq len 77. padding:true alone often leaves short seqs (e.g. 18) with Xenova/HF — UNet then errors "Expected: 77".
            const CLIP_SEQ = 77;
            const tokOut = await tok(prompt, {
                padding: 'max_length',
                max_length: CLIP_SEQ,
                truncation: true,
                return_tensor: false,
            });
            let rawIds = tokOut.input_ids;
            if (rawIds && typeof rawIds === 'object' && rawIds.data)
                rawIds = rawIds.data;
            const flat = Array.isArray(rawIds) ? rawIds : Array.from(rawIds);
            const padId = typeof tok.pad_token_id === 'number' ? tok.pad_token_id : 0;
            const ids = new Int32Array(CLIP_SEQ);
            const n = Math.min(flat.length, CLIP_SEQ);
            for (let i = 0; i < n; i++)
                ids[i] = Number(flat[i]);
            for (let i = n; i < CLIP_SEQ; i++)
                ids[i] = padId;
            // Text encoder
            onProgress?.({ phase: 'encoding', pct: 25 });
            let encOut;
            try {
                encOut = await this.sessions.text_encoder.run({ input_ids: new ort.Tensor('int32', ids, [1, CLIP_SEQ]) });
            }
            catch (e) {
...
async function getTokenizer() {
    ...
    try {
        ...
        try {
            ...
            try {
                const mod2 = await import(/* webpackIgnore: true */ HF_TRANSFORMERS_ESM);
